...
public class Main {
    ...
    public static void main(String[] args) throws LifecycleException {
        ...
        if (!webappFile.exists()) {
            webappFile = new File("webapp_static");
        }
        if (!webappFile.exists()) {
        ...
        // OPTIMIZATION: Disable JAR scanning to eliminate NoSuchFileException warnings and speed up startup.
        // Since we manually register all servlets and filters, we don't need Tomcat to scan the classpath.
        org.apache.tomcat.JarScanner scanner = ctx.getJarScanner();
        if (scanner instanceof org.apache.tomcat.util.scan.StandardJarScanner stdScanner) {
            stdScanner.setScanClassPath(false);
            stdScanner.setScanAllDirectories(false);
            stdScanner.setScanBootstrapClassPath(false);
            stdScanner.setScanManifest(false);
        }
        ...
        // FIX: Manually register AppFilter for SPA routing and CSS injection
        // This is required because embedded Tomcat doesn't scan for @WebFilter by default.
        org.apache.tomcat.util.descriptor.web.FilterDef filterDef = new org.apache.tomcat.util.descriptor.web.FilterDef();
        filterDef.setFilterName("UnifiedAppFilter");
        filterDef.setFilterClass(com.billing.filter.AppFilter.class.getName());
        filterDef.setFilter(new com.billing.filter.AppFilter());
        ctx.addFilterDef(filterDef);

        org.apache.tomcat.util.descriptor.web.FilterMap filterMap = new org.apache.tomcat.util.descriptor.web.FilterMap();
        filterMap.setFilterName("UnifiedAppFilter");
        filterMap.addURLPattern("/*");
        filterMap.setDispatcher(jakarta.servlet.DispatcherType.REQUEST.name());
        filterMap.setDispatcher(jakarta.servlet.DispatcherType.FORWARD.name());
