  ...
  <div class="page-header">
    ...
    <div class="admin-tabs-container">
      <div class="admin-tabs">
        <button class="admin-tab-btn" class:active={activeTab === 'dashboard'} onclick={() => activeTab = 'dashboard'}>
        ...
        <button class="admin-tab-btn" class:active={activeTab === 'rateplans'} onclick={() => activeTab = 'rateplans'}>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>
          Rate Plans
        </button>
        <button class="admin-tab-btn" class:active={activeTab === 'packages'} onclick={() => activeTab = 'packages'}>
        ...
        <div class="tab-pill-bg" style="transform: translateX({activeTab === 'dashboard' ? '0' : activeTab === 'rateplans' ? '100%' : '200%'})"></div>
      </div>
    </div>
  </div>
  ...
  {#key activeTab}
    <div in:fade={{ duration: 250, delay: 100 }}>
      {#if activeTab === 'dashboard'}
        <!-- System Status Bar -->
        <div class="system-status-bar" class:status-offline={systemStatus === 'Offline'}>
        ...
        <div class="stats-bento-grid stats-container">
          <div class="card contract-gradient hero-card">
          ...
          <div class="card customer-gradient">
        ...
        <div class="quick-actions-section">
          ...
          <div class="grid-admin">
            ...
            <a href="/admin/billing" class="action-card card">
              ...
              <p>Invoices & financial integrity</p>
            </a>
            <button onclick={() => activeTab = 'rateplans'} class="action-card card">
              <div class="icon-pod violet">
            ...
            <button onclick={() => activeTab = 'packages'} class="action-card card">
              ...
              <h3>Offer Portfolio</h3>
              <p>Voice, data & SMS bundles</p>
            </button>
            <a href="/admin/cdr" class="action-card card">
              <div class="icon-pod blue">
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
              </div>
              <h3>Usage Analytics</h3>
              <p>Advanced CDR & trend analysis</p>
            </a>
          </div>
        </div>
...
<style>
  ...
  .admin-tabs-container { display: flex; align-items: center; }
  .admin-tabs { position: relative; display: flex; background: rgba(0,0,0,0.4); padding: 4px; border-radius: 14px; border: 1px solid var(--border); overflow: hidden; }
  .admin-tab-btn { position: relative; z-index: 2; padding: 10px 20px; border: none; background: none; color: var(--text-muted); font-weight: 700; font-size: 0.85rem; border-radius: 10px; cursor: pointer; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); display: flex; align-items: center; gap: 8px; }
  .admin-tab-btn:hover { color: white; }
  .admin-tab-btn.active { color: white; }
  .tab-pill-bg { position: absolute; top: 4px; left: 4px; width: calc(33.33% - 2.66px); height: calc(100% - 8px); background: var(--red); border-radius: 10px; z-index: 1; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 4px 15px rgba(224, 8, 0, 0.3); }
  ...
  .card-content-hero { display: flex; flex-direction: column; gap: 0.5rem; padding: 0.1rem; }
  .hero-header { display: flex; align-items: center; gap: 1.75rem; }
  .stat-value-large { font-size: 1.5rem; font-weight: 900; letter-spacing: -0.02em; line-height: 1; color: white; }
  ...
  .icon-circle { width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: rgba(255, 255, 255, 0.05); }
  
  .stat-info-centered { display: flex; flex-direction: column; align-items: center; width: 100%; }
  .stat-value { font-size: 1.25rem; font-weight: 800; letter-spacing: -0.01em; line-height: 1.1; }
  ...
  .quick-actions-section { border-top: 1px solid var(--border); padding-top: 1.5rem; }
  ...
  .action-icon-box { 
    width: 32px; height: 32px; background: rgba(224, 8, 0, 0.1); 
    border-radius: 10px; display: flex; align-items: center; justify-content: center; 
    margin-bottom: 0.5rem; color: var(--red);
  }
